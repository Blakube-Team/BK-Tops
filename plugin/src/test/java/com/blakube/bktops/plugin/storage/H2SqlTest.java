...
class H2SqlTest {
    ...
    private String trimSql() {
        return "DELETE FROM " + TABLE + " WHERE identifier NOT IN " +
               "(SELECT identifier FROM (SELECT identifier FROM " + TABLE + " ORDER BY top_value DESC, last_updated ASC, identifier ASC LIMIT ?) AS keep_list)";
    }
    ...
    private List<String> loadTopIds() throws SQLException {
        ...
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(
                "SELECT identifier FROM " + TABLE + " ORDER BY top_value DESC, last_updated ASC, identifier ASC")) {
    ...
    private String positionSql() {
        return "SELECT (SELECT COUNT(*) + 1 FROM " + TABLE + " WHERE top_value > t.top_value OR (top_value = t.top_value AND last_updated < t.last_updated) OR (top_value = t.top_value AND last_updated = t.last_updated AND identifier < t.identifier)) as position " +
